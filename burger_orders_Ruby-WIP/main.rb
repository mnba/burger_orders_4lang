#!/usr/bin/ruby
=begin
 Process burger orders, 4 languages. Ruby: 5th language
 Tested with ruby v.?
=end

class OneBurger
	def initialize(bun_ty, addit_fillings)
		@bun_type = bun_ty
		@addit_fillings = addit_fillings
		@sum = 0.
	end
end

class BurgerOrder
	def initialize(burgers_list, delivery_type)
		@burgers_list = burgers_list
...
def get_bun_cost(bun_type)
	if not bun_costs.has_key?(bun_type)
		...
		raise RuntimeError, "Bun type unknown: "+bun_type
		return 0
	end
...
def get_filling_cost(filling)
	if not filling_costs.has_key?(filling)
		...
		raise RuntimeError, "Filling unknown: "+filling
		return 0
	end
...
def calc_one_burger_order(one_burger_order1)
	...
	if n > 5
		raise RuntimeError, "Maximum 5 fillings are allowed, found #{n} fillings"
	end
	sum = 0.
